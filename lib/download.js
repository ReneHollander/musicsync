...
const exePath = path.resolve(process.cwd(), './bin/youtube-dl' + (isWin ? '.exe' : ''));
...
module.exports.fetchPlaylist = async function (data_path, name, playlist_url) {
    ...
    if (await fs.pathExists(downloaded_path)) {
        ...
        let {stdout} = await execFile(exePath, ['-j', '--flat-playlist', playlist_url], {cwd: data_path});
        ...
        let [newDownloaded, toDelete] = _.partition(oldDownloaded, id => !contains(ids, id));
        toDelete = toDelete.map(line => line.split(' ')[1]);
        ...
        await fs.writeFile(downloaded_path, newDownloaded.join(os.EOL));
    }
    ...
    await execFile(exePath, [
        // Put as much metadata into the mp3 as possible.
        '--metadata-from-title', '%(artist)s - %(title)s', '--add-metadata', '--embed-thumbnail',
        // Make sure the track is always a mp3 file. If it gets converted, it will have 256K bitrate.
        '--audio-quality', '256K', '--extract-audio', '--audio-format', 'mp3',
    ...
    for (let info_file of info_files) {
        let audio_path = replaceExt(info_file, '.mp3');
        ...
        tracks[info.playlist_index - 1] = new Track(audio_path, info);
    }
